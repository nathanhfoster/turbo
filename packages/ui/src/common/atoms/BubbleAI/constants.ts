...
export const QUALITY_SETTINGS: Record<
  BubbleAIQuality,
  { particles: number; smoothness: string }
> = {
...
export function getDefaultRings(
  ...
  const baseRings: RingConfig[] = RING_THEMES.nebula[theme].map((ring) => ({
    ...ring,
  }));
  ...
  while (rings.length < requestedCount) {
    ...
    const newRing: RingConfig = {
      ...
      rotationSpeed:
        (baseRing.rotationSpeed ?? 1) *
        (variationIndex % 2 === 0 ? 1 : -1) *
        (0.8 + variationIndex * 0.1),
      glowIntensity: Math.max(
        0.3,
        (baseRing.glowIntensity ?? 0.6) - variationIndex * 0.1,
      ),
...
export function generateParticles(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const orbitSpeed =
      personality === "explorer"
        ? (getRandomNumber(50, 150) / 1000) *
          (getRandomNumber(0, 1) === 0 ? 1 : -1)
        : undefined;
...
export function regenerateParticle(size: number): {
  ...
  const energyBurst =
    personality === "energetic" ? getRandomNumber(1000, 3000) : undefined;
