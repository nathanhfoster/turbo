...
export function SitemapViewPage({
  sitemaps,
  baseUrl,
  title = "Sitemap",
  description = "Visual sitemap of all pages",
}: SitemapViewPageProps) {
      ...
      {sitemaps.map((sitemap) => (
        <Card key={sitemap.id} className="mb-8 p-6" hoverable>
          ...
          <Box className="space-y-6">
            ...
            {Array.from(new Set(sitemap.entries.map((e) => e.type))).map(
              (type) => {
                const entriesOfType = sitemap.entries.filter(
                  (e) => e.type === type,
                );
                return (
                  <Box key={type}>
                    ...
                    >
                      {type} Routes ({entriesOfType.length})
                    </Typography>
