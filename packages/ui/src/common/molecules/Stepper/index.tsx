...
const Stepper: React.FC<StepperProps> = ({
    ...
    (index: number) => {
      if (orientation === 'horizontal') {
        return combineClassNames(
          ...
          index <= currentStep ? activeColors.active.text : inactiveColors.inactive.text
        );
      }
      return combineClassNames('mb-10 ms-6', index === data.length - 1 && 'mb-0');
    },
  ...
  return (
    <Box variant="ol" className={combineClassNames(orientationClasses, className)}>
      {data.map((step, index) => {
          ...
          <li key={index} className={getStepClasses(index)}>
            <div className={getStepIndicatorClasses(isCompleted, isActive)}>
              {isCompleted ? (
                ...
                <Typography
                  size="text-sm"
                  className={isActive ? activeColors.active.text : inactiveColors.inactive.text}
                >
                  {index + 1}
                </Typography>
              )}
            ...
            <Box className="flex flex-col">
              <Typography
                ...
                className={combineClassNames(
                  ...
                  isActive ? activeColors.active.text : inactiveColors.inactive.text
                )}
              >
                {step.label}
              </Typography>
