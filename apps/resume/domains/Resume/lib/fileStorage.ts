 ...
 */
export function resumeFileDataToFile(fileData: Resume["fileData"]): File | null {
  ...
  try {
    ...
    const blob = fileData.data instanceof Blob 
      ? fileData.data 
  ...
  for (const resume of resumes) {
    if (resume.fileData) {
      ...
      if (file) {
        ...
        console.warn(`Failed to convert fileData to File for resume: ${resume.name}`);
      }
    }
  }
