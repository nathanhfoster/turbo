...
export const calculateColumnWidths = <T>(columns: TableColumn<T>[], totalWidth: number) => {
  ...
  const totalFixedWidth = fixedWidthColumns.reduce((sum, col) => sum + (col.width || 0), 0);
  ...
  let columnsWithConstraints = columns.map(column => {
    const baseWidth = column.width !== undefined ? column.width : initialFlexibleWidth;
