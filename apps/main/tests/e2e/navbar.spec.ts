...
test.describe("Navbar Functionality", () => {
  ...
  test.describe("Bottom Navbar", () => {
    ...
    test("should highlight active page in bottom navbar", async ({ page }) => {
      ...
      const hasActiveClass = classList.some(
        (cls) => cls.includes("primary") || cls.includes("active"),
      );
  ...
  test.describe("Hamburger Menu", () => {
    test("should display hamburger menu on mobile", async ({ page }) => {
      ...
      const hamburgerButton = page
        ...
        .filter({
          has: page.locator(
            'svg, [aria-label*="menu" i], [aria-label*="Menu"]',
          ),
        })
        .first();
    ...
    test("should open menu when hamburger is clicked", async ({ page }) => {
      ...
      if (buttonCount > 0) {
        ...
        const drawer = page.locator(
          '[role="dialog"], [data-testid*="menu"], [class*="drawer"]',
        );

        // Either menu items are visible or drawer is open
        const hasMenu =
          (await menuItems.count()) > 0 || (await drawer.count()) > 0;
  ...
  test.describe("Scroll Behavior", () => {
    ...
    test("should hide bottom navbar when scrolling down on mobile", async ({
      page,
    }) => {
      // Set viewport to mobile size
      await page.setViewportSize({ width: 375, height: 667 });
      ...
      const isHidden =
        (transform.includes("translateY") &&
          !transform.includes("translateY(0px)")) ||
    ...
    test("should show bottom navbar when scrolling up on mobile", async ({
      page,
    }) => {
      // Set viewport to mobile size
      await page.setViewportSize({ width: 375, height: 667 });
    ...
    test("should maintain navbar visibility at top of page", async ({
      page,
    }) => {
      await page.setViewportSize({ width: 1280, height: 720 });
      await page.waitForTimeout(500);
    ...
    test("should hide both navbars when scrolling down on mobile", async ({
      page,
    }) => {
      // Set viewport to mobile size
      await page.setViewportSize({ width: 375, height: 667 });
      ...
      const topIsHidden =
        topTransform !== "none" &&
        !topTransform.includes("matrix(1, 0, 0, 1, 0, 0)");
      const bottomIsHidden =
        bottomTransform !== "none" &&
        !bottomTransform.includes("matrix(1, 0, 0, 1, 0, 0)");
